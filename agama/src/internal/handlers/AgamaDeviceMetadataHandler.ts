...
import {
  ...
  getAgamaDeviceId, mapAgamaMobileConnectionType,
} from '../AgamaUtils';
...
export class AgamaDeviceMetadataHandler {
  ...
  constructor(private readonly empClient: Agama.EMPClient, private readonly agama: typeof Agama) {
  }
  ...
  async signalDeviceMetadata_(agamaPlayerConfig: AgamaConfiguration, agamaPlayerDescription: AgamaPlayerDescription): Promise<void> {
    ...
    this.reportOsInfo_();
    this.empClient.setDeviceMetadata(
      this.agama.DeviceMetadata.EMPCLIENT_INTEGRATION_VERSION,
      EMPCLIENT_INTEGRATION_VERSION
    );
    this.empClient.setDeviceMetadata(
      this.agama.DeviceMetadata.EMPCLIENT_INTEGRATION_BUILDDATE,
      EMPCLIENT_INTEGRATION_BUILDDATE
    );
    ...
    if (agamaPlayerConfig.applicationVersion) {
      this.empClient.setDeviceMetadata(
        this.agama.DeviceMetadata.APPLICATION_VERSION,
        agamaPlayerConfig.applicationVersion
      );
    }
    if (agamaPlayerConfig.userAccountID) {
      this.empClient.setDeviceMetadata(
        this.agama.DeviceMetadata.USER_ACCOUNT_ID,
        agamaPlayerConfig.userAccountID
      );
    }
  ...
  private reportBrowserInfo_(): void {
    ...
    if (browserMajorVersion !== undefined) {
      this.empClient.setDeviceMetadata(
        this.agama.DeviceMetadata.DEVICE_BROWSER_VERSION,
        browserMajorVersion.toString()
      );
    }
  }
  ...
  private readonly reportConnectionType_ = (state: NetInfoState): void => {
    ...
    switch (connectionType) {
      case NetInfoStateType.wifi:
        this.empClient.setDeviceMetadata(
          this.agama.DeviceMetadata.DATA_CONNECTION_TYPE,
          'wlan'
        );
        break;
