...
function isAgamaConfiguration(sourceConfiguration: AnalyticsDescription): sourceConfiguration is AgamaSourceConfiguration {
  return sourceConfiguration &&
    sourceConfiguration.integration === AGAMA_ANALYTICS_INTEGRATION_ID;
}
...
export function isAgamaLogLevelType(logLevel: any): logLevel is AgamaLogLevelType {
  ...
  return logLevel === 'info' || logLevel === 'debug' || logLevel === 'warning'
...
export function isAgamaServiceName(serviceName: any): serviceName is AgamaServiceName {
  ...
  return serviceName === 'live' || serviceName === 'svod' || serviceName === 'nvod' || serviceName === 'tvod'
    || serviceName === 'avod' || serviceName === 'catchuptv';
}
...
function sanitiseAgamaSourceConfiguration(configuration: AgamaSourceConfiguration): AgamaSourceConfiguration {
  ...
  if (configuration.serviceName !== undefined && !isAgamaServiceName(configuration.serviceName)) {
    throw new TypeError(`Agama source configuration: expected 'serviceName' to be 'live', 'svod', 'nvod',`
      + `'tvod', 'avod' or 'catchuptv'`);
  }
...
function sanitiseAgamaPlayerConfiguration(configuration: AgamaConfiguration): AgamaConfiguration {
  ...
  if (configuration.logLevel !== undefined && !isAgamaLogLevelType(configuration.logLevel)) {
    throw new TypeError(`Agama player configuration: expected 'logLevel' to be 'info', 'debug', 'warning',`
      + `'error' or 'fatal'`);
  }
...
export function getAgamaSourceConfiguration(analytics: AnalyticsDescription[] = []):
  AgamaSourceConfiguration | undefined {
...
export function getAgamaPlayerConfiguration(analytics: AgamaConfiguration):
  AgamaConfiguration | undefined {
...
export function generateAgamaAbrConfiguration(agama: typeof Agama,
