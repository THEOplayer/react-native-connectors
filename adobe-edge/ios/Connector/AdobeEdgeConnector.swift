...
class AdobeEdgeConnector {
  ...
  init(player: THEOplayer, baseUrl: String, configId: String, userAgent: String?, debug: Bool, debugSessionId: String?) {
    ...
    self.mediaApi = MediaEdgeAPI(baseUrl: baseUrl, configId: configId, userAgent: userAgent ?? buildUserAgent(), debugSessionId: debugSessionId)
  ...
  func setDebugSessionId(_ id: String?) -> Void {
    mediaApi.setDebugSessionId(id: id)
  }
  
  func updateMetadata(_ metadata: [AdobeCustomMetadataDetails]) -> Void {
    customMetadata.append(contentsOf: metadata)
  }
  
  func setError(_ errorDetails: AdobeErrorDetails) -> Void {
    guard let player = self.player else {return}
    mediaApi.error(playhead: player.currentTime, errorDetails: errorDetails)
  }
  ...
  func onPlaying(event: PlayingEvent) -> Void {
    ...
    maybeStartSession(mediaLengthSec: player.duration) {
  ...
  func onPause(event: PauseEvent) -> Void {
    ...
    mediaApi.pause(playhead: player.currentTime)
  }
  
  func onWaiting(event: WaitingEvent) -> Void {
    guard let player = self.player else {return}
    self.log("onWaiting triggered.")
    mediaApi.bufferStart(playhead: player.currentTime)
  }
  
  func onEnded(event: EndedEvent) -> Void {
    guard let player = self.player else {return}
    self.log("onEnded triggered.")
    mediaApi.sessionComplete(playhead: player.currentTime)
    self.reset()
  }
  ...
  func onActiveQualityChange(event: ActiveQualityChangedEvent) -> Void {
    ...
    mediaApi.bitrateChange(
      playhead: player.currentTime, qoeDataDetails: AdobeQoeDataDetails(
        bitrate: 0 // TODO?
      )
    )
  }
  ...
  func onAdBreakBegin(event: AdBreakBeginEvent) -> Void {
    ...
    let podDetails = calculateAdvertisingPodDetails(adBreak: event.ad, lastPodIndex: self.adBreakPodIndex)
    mediaApi.adBreakStart(playhead: player.currentTime, advertisingPodDetails: podDetails)
  ...
  func onAdBreakEnd(event: AdBreakEndEvent) -> Void {
    ...
    mediaApi.adBreakComplete(playhead: player.currentTime)
  }
  ...
  func onAdBegin(event: AdBeginEvent) -> Void {
    ...
    mediaApi.adStart(
      playhead: player.currentTime,
      advertisingDetails: calculateAdvertisingDetails(ad: event.ad, podPosition: adPodPosition),
  ...
  func onAdEnd(event: AdEndEvent) -> Void {
    ...
    mediaApi.adComplete(playhead: player.currentTime)
  }
   ...
   * @private
   */
  func maybeStartSession(mediaLengthSec: Double? = nil, completion: (()->Void)? = nil) -> Void {
    ...
    Task { @MainActor in
      ...
      guard mediaApi.hasSessionStarted() else {
        self.log("maybeStartSession - session was not started")
        return
      }
      
      sessionInProgress = true
      self.log("maybeStartSession - STARTED sessionId: \(mediaApi.sessionId ?? "<no session>")")
  ...
  private func maybeEndSession(completion: (()->Void)? = nil) -> Void {
    ...
    if (mediaApi.hasSessionStarted()) {
      mediaApi.sessionEnd(playhead: player.currentTime)
    }
    reset()
  }
