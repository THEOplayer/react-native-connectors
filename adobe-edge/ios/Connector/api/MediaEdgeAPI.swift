...
class MediaEdgeAPI {
    ...
    func setDebugSessionId(debugId: String?) {
        debugSessionId = debugId
    }
    ...
    private func sendRequest(url: String, body: String) async throws -> Data? {
        ...
        do {
            ...
            print("Adobe sendRequest responseCode: \(httpResponse)")
            return data
        } catch {
            throw error
        }
    ...
    func startSession(
        ...
        do {
            ...
            mediaCollection["sessionDetails"] = sessionDetails.toJSONEncodableDictionary()
            if let qoeData = qoeDataDetails {
                mediaCollection["qoeDataDetails"] = qoeData.toJSONEncodableDictionary()
            }
            if let metadata = customMetadata {
                mediaCollection["customMetadata"] = metadata.map({ $0.toJSONEncodableDictionary() })
            }
            let xdm: [String:Any] = [
            ...
            if let responseData = responseData,
               let jsonResponse = try? JSONSerialization.jsonObject(with: responseData) as? [String: Any] {
                ...
                if let handle = jsonResponse["handle"] as? [[String: Any]] {
                    let sessionIdHandle = handle.first { handleItem in
                        handleItem["type"] as? String == "media-analytics:new-session"
                    }
                    
                    if let handle = sessionIdHandle,
                       let payload = handle["payload"] as? [[String:Any]],
                       let sessionId = payload.first?["sessionId"] as? String {
                        self.sessionId = sessionId
                        print("Adobe sessionId received: \(sessionId)")
                    }
                }
            }
    ...
    private func postEvent(sessionId: String, path: String, mediaDetails: [String: Any?]) async {
        do {
            var mediaCollection: [String:Any] = [:]
            mediaCollection["playhead"] = 0
            if let qoeData = mediaDetails["qoeDataDetails"] as? AdobeQoeDataDetails {
                mediaCollection["qoeDataDetails"] = qoeData.toJSONEncodableDictionary()
            }
            if let metadata = mediaDetails["customMetadata"] as? [AdobeCustomMetadataDetails] {
                mediaCollection["customMetadata"] = metadata.map({ $0.toJSONEncodableDictionary() })
            }
            if let advertisingPodDetails = mediaDetails["advertisingPodDetails"] as? AdobeAdvertisingPodDetails {
                mediaCollection["advertisingPodDetails"] = advertisingPodDetails.toJSONEncodableDictionary()
            }
            if let advertisingDetails = mediaDetails["advertisingDetails"] as? AdobeAdvertisingDetails {
                mediaCollection["advertisingDetails"] = advertisingDetails.toJSONEncodableDictionary()
            }
            if let chapterDetails = mediaDetails["chapterDetails"] as? AdobeChapterDetails {
                mediaCollection["chapterDetails"] = chapterDetails.toJSONEncodableDictionary()
            }
            if let errorDetails = mediaDetails["errorDetails"] as? AdobeErrorDetails {
                mediaCollection["errorDetails"] = errorDetails.toJSONEncodableDictionary()
            }
