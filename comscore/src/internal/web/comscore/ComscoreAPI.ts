...
export class ComscoreAPI {
  ...
  constructor(
    ...
    if (!(window as any).ns_) {
      throw new Error('There is comscore library reference missing!');
    }
    if (!publisherId) {
      throw new Error('publisherId is mandatory parameter!');
    }
    this.analytics = (window as any).ns_.analytics;
    if (skeleton) {
      this.useSkeletonApi = true;
      this.skeletonInterface = skeleton;
    }
  ...
  setPlatformAPI() {
    if (this.skeletonInterface) {
      this.analytics.PlatformApi.setPlatformAPI(
        this.analytics.PlatformAPIs.Skeleton,
      ...
      this.analytics.PlatformApi.setPlatformAPI(
        this.analytics.PlatformAPIs.WebBrowser
      );
      if (this.showLogs()) {
  ...
  configurePublisher(publisherId: string) {
    const config = new this.analytics.configuration.PublisherConfiguration({
      publisherId,
    });
    this.analytics.configuration.addClient(config);
    if (this.showLogs()) {
