...
export class AdMetadata {
  ...
  constructor(
    ...
    const cm = new (
      window as any
    ).ns_.analytics.StreamingAnalytics.AdvertisementMetadata();
   ...
   */
  public setMediaType(adType: AD_TYPES, isLive: boolean) {
    const {
      ON_DEMAND_PRE_ROLL,
      ON_DEMAND_MID_ROLL,
      ON_DEMAND_POST_ROLL,
      LIVE,
    } = (window as any).ns_.analytics.StreamingAnalytics.AdvertisementMetadata
      .AdvertisementType;
