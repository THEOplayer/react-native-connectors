...
export abstract class TheoBase {
   ...
   */
  protected abstract bindListeners();

  /**
   * This method should be used to handle source change
   */
  protected abstract notifySourceChange();
   ...
   */
  protected abstract notifyAdBegin(adId, adType, adObject);

  /**
   * Tis method should be used to notify once the stream playout ends
   */
  protected abstract notifyEndOfStream();

  /**
   * Tis method should be used to notify once ad ends
   */
  protected abstract notifyAdEnd(adType);

  /**
   * notifies that buffer starts
   * @param {*} currentTime in sec
   */
  protected abstract notifyBufferStart(currentTime);

  /**
   * notifies that buffer stops
   * @param {*} currentTime in sec
   */
  protected abstract notifyBufferStop(currentTime);

  /**
   * notifies play starts
   * @param {*} currentTime in sec
   */
  protected abstract notifyPlay(currentTime);

  /**
   * notifies pause
   * @param {*} currentTime in sec
   */
  protected abstract notifyPause(currentTime);

  /**
   * notifies seeking starts
   * @param {*} currentTime in sec
   */
  protected abstract notifySeekStart(currentTime);

  /**
   * notifies data loaded
   */
  protected abstract notifyDataLoaded();

  /**
   * notifies about rate change
   */
  protected abstract notifyRateChange(rateChange);
  ...
  private onLoadedData() {
    this.addListener(this.player, THEO_EVENTS.LOADED_DATA, (event) => {
  ...
  private onTimeUpdate() {
    this.addListener(this.player, THEO_EVENTS.TIME_UPDATE, (event) => {
   ...
   */
  protected onAdBegin() {
    this.addListener(this.player.ads, THEO_EVENTS.AD_BEGIN, (event) => {
      //console.log(['THEOBASE onAdBegin']);
      this.adPlayheadPosition = 0;
  ...
  protected onBuffering() {
    this.addListener(this.player, THEO_EVENTS.READY_STATE_CHANGE, (event) => {
      ...
      if (!this.simpleBufferNotifications) {
        switch (readyState) {
          ...
          case HAVE_METADATA:
            ...
            if (!this.simpleBufferNotifications)
              this.notifyBufferStart(currentTime);
            break;
          case HAVE_ENOUGH_DATA:
            this.isBuffering = false;
            if (!this.simpleBufferNotifications)
              this.notifyBufferStop(currentTime);
            break;
   ...
   */
  protected onPlay() {
    this.addListener(this.player, THEO_EVENTS.PLAY, (event) => {
   ...
   */
  protected onPlaying() {
    this.addListener(this.player, THEO_EVENTS.PLAYING, (event) => {
   ...
   */
  protected onPause() {
    this.addListener(this.player, THEO_EVENTS.PAUSE, (event) => {
   ...
   */
  protected onSeeking() {
    this.addListener(this.player, THEO_EVENTS.SEEKING, (event) => {
      const { currentTime } = event;
  ...
  protected onRateChange() {
    this.addListener(this.player, THEO_EVENTS.RATE_CHANGE, (event) => {
   ...
   */
  private addListener(caller, type, f) {
